export default async function handler(request) {
  ...
  const code =
    callbackUrl.searchParams.get('authorization_code') ||
  ...
  if (!tokenResponse.ok || !tokenData.access_token) {
    ...
    return new Response('知乎授权失败，请查看 Netlify Function 日志', {
      status: 502,
    });
  }

  const userInfo = await fetchZhihuUserInfo(
  ...
  if (!userInfo.ok) {
    return new Response(
      `知乎授权成功，但未能读取用户资料：${userInfo.error}`,
      { status: 502 }
    );
  }

  const saved = await saveToSupabase(userInfo.data, tokenData);

  if (!saved.ok) {
    return new Response(
      `知乎授权成功，但未能保存到 Supabase：${saved.error}`,
      { status: 502 }
    );
  }
...
async function fetchZhihuUserInfo(accessToken, tokenType) {
  ...
  if (!response.ok) {
    ...
    return {
      ok: false,
      error: `知乎用户信息接口返回 HTTP ${response.status}`,
    };
  }
...
async function saveToSupabase(userInfo, tokenData) {
  ...
  if (!supabaseUrl || !serviceKey) {
    return {
      ok: false,
      error: 'Netlify 中缺少 Supabase 环境变量',
    };
  }

  const user = userInfo.data || userInfo.Data || userInfo;
  ...
  const userResponse = await fetch(
    `${base}/users?on_conflict=zhihu_user_id`,
    {
      method: 'POST',
      headers: {
        ...headers,
      ...
      body: JSON.stringify({
        ...
        profile_url:
          user.url ||
          user.profile_url ||
          (user.url_token || user.urlToken
            ? `https://www.zhihu.com/people/${user.url_token || user.urlToken}`
            : null),
  ...
  if (
    ...
    return {
      ok: false,
      error: `users 表写入失败（HTTP ${userResponse.status}）`,
    };
  }

  const accountResponse = await fetch(
    `${base}/zhihu_accounts?on_conflict=user_id`,
    {
      method: 'POST',
      headers: {
        ...headers,
  ...
  if (!accountResponse.ok) {
    return {
      ok: false,
      error: `zhihu_accounts 表写入失败（HTTP ${accountResponse.status}）`,
    };
  }
...
export const config = {
  path: '/.netlify/functions/zhihu-callback',
};
